  ...
  describe('fetchHikes', () => {
    ...
    it('dispatches FETCH_HIKES_SUCCESS action when promise gets resolved', () => {
      ...
      return actions.fetchHikes()(dispatch, null, api).then(() => {
        expect(dispatch).to.have.been.calledWith({type: 'FETCH_HIKES_SUCCESS', response: expectedHikes});
      });
    });
    ...
    it('dispatches FETCH_HIKES_ERROR action when promise gets rejected', () => {
      ...
      return actions.fetchHikes()(dispatch, null, api).then(() => {
        expect(dispatch).to.have.been.calledWith({type: 'FETCH_HIKES_FAILURE', message: "some message"});
      });
    });
  });
